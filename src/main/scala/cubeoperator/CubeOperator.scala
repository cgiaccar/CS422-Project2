 ...
 * */
  def cube(dataset: Dataset, groupingAttributes: List[String], aggAttribute: String, agg: String): RDD[(String, Double)] = {
    ...
    if (agg == "AVG") {
      val topCuboid: RDD[(List[Any], (Double, Int))] = rdd.map(e => (indexes.map(i => e.get(i)), (e.getInt(indexAgg).toDouble, 1))).reduceByKey((d1, d2) => (d1._1 + d2._1, d1._2 + d2._2), reducers).persist()
      val cube: RDD[(String, Double)] = topCuboid.flatMap { e =>
        ...
        for (r <- partialCubeLattices) yield (r.map(i => i).toString, e._2)
      }.reduceByKey((d1,d2) => (d1._1 + d2._1, d1._2 + d2._2), reducers).map{case (k, v) => (k, v._1/v._2)}
      cube
    } else {
      val topCuboid: RDD[(List[Any], Double)] = agg match {
        case "COUNT" =>
         rdd.map(e => (indexes.map(i => e.get(i)), 1.0)).reduceByKey(_+_, reducers).persist()
        case "SUM" =>
          rdd.map(e => (indexes.map(i => e.get(i)), e.getInt(indexAgg).toDouble)).reduceByKey(_+_, reducers).persist()
        case "MIN" =>
          rdd.map(e => (indexes.map(i => e.get(i)), e.getInt(indexAgg).toDouble)).reduceByKey((d1,d2) => math.min(d1,d2), reducers).persist()
        case "MAX" =>
          rdd.map(e => (indexes.map(i => e.get(i)), e.getInt(indexAgg).toDouble)).reduceByKey((d1,d2) => math.max(d1,d2), reducers).persist()
      }
      val cube: RDD[(String, Double)] = topCuboid.flatMap { e =>
        val partialCubeLattices: List[List[Any]] = (for (i <- 0 to e._1.size) yield e._1.combinations(i)).flatten.toList
        for (r <- partialCubeLattices) yield (r.map(i => i).toString, e._2)
